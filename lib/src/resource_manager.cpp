...
namespace rst
{
    ...
    auto resource_manager::load_texture( std::filesystem::path const& path ) -> std::shared_ptr<pelt>
    {
        earmark const mark{ path.string( ) };
        std::filesystem::path const full_path{ data_path_ / path };
        if ( not loaded_textures_.contains( mark ) )
        {
            // loaded_textures_.insert( std::pair( mark, std::make_shared<pelt>( full_path.string( ) ) ) );
        }
